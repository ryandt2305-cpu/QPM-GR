...
let shopBuyCount = 0;
let harvestStatusTimer: number | null = null;
let harvestHeaderStatusText = '';
let harvestSummaryUnsubscribe: (() => void) | null = null;
let harvestToastUnsubscribe: (() => void) | null = null;
...
const HARVEST_MUTATION_DEFAULTS: Record<HarvestMutationKey, boolean> = {
  Rainbow: true,
  Gold: false,
  Frozen: false,
  Wet: false,
...
interface HarvestMutationOption {
  key: HarvestMutationKey;
  label: string;
  description: string;
}
...
const HARVEST_MUTATION_OPTIONS: readonly HarvestMutationOption[] = [
  ...
  { key: 'Amberlit', label: 'Amberlit', description: 'Amber weather active' },
  { key: 'Amberbound', label: 'Amberbound', description: 'Amber weather with charged plants' },
  { key: 'Dawnbound', label: 'Dawnbound', description: 'Dawn weather with charged plants' },
];
  ...
  for (const view of shopCountdownViews) {
    ...
    for (const cat of SHOP_CATEGORY_DEFINITIONS) {
      ...
      if (!autoShopEnabled) {
        ...
        summaryParts.push(`${cat.icon} off`);
        continue;
      }
...
function createWeatherSection(): HTMLElement {
  ...
  const { root, body } = createCard('🌦️ Weather Tracking', {
    collapsible: true,
    startCollapsed: true,
  });
  ...
  infoText.textContent = 'Weather tracking is available in the Mutation Tracker for identifying optimal harvesting times.';
  infoText.style.padding = '8px 0';
...
export function createOriginalUI(): HTMLElement {
  ...
  if (weatherButton) {
    ...
    newWeatherButton.addEventListener('click', () => {
      toggleWindow('weather-events', '🌦️ Weather & Events', renderWeatherEventsWindow, '650px', '85vh');
    });
  ...
  if (shopButton) {
    ...
    newShopButton.addEventListener('click', () => {
      toggleWindow('shop-locker', '🛒 Shop & Locker', renderShopLockerWindow, '650px', '85vh');
    });
...
function createStatsHeader(): HTMLElement {
  ...
  const row = document.createElement('div');
  row.className = 'qpm-grid qpm-grid--two';
  container.appendChild(row);

  const autoValue = createHeaderSegment(row, '🍖 Auto Feed');
  const weatherValue = createHeaderSegment(row, '☀️ Weather');
  const shopValue = createHeaderSegment(row, '🛒 Shop');

  const meta = document.createElement('div');
  meta.className = 'qpm-section-muted';
  meta.style.cssText = 'margin-top: 8px; font-size: 10px; color: #a0a8c0;';
  container.appendChild(meta);

  uiState.headerContainer = container;
  uiState.headerAutoFeed = autoValue;
  uiState.headerWeather = weatherValue;
  uiState.headerShop = shopValue;
  uiState.headerMeta = meta;

  if (uiState.headerTimer != null) {
    window.clearInterval(uiState.headerTimer);
    uiState.headerTimer = null;
  }

  refreshHeaderStats();
  uiState.headerTimer = window.setInterval(refreshHeaderStats, 1500) as unknown as number;

  // Add divider
  const divider = document.createElement('div');
  divider.style.cssText = 'height:1px;background:rgba(143,130,255,0.15);margin:12px 0;';
  ...
  // === Pet Hunger Indicator ===
  const hungerCard = createMinimalCard('🍖', 'Pet Hunger', 'rgba(255,152,0,0.15)');
  const hungerList = document.createElement('div');
  hungerList.style.cssText = 'display:flex;flex-direction:column;gap:3px;font-size:11px;color:#e0e0e0;';
  hungerCard.appendChild(hungerList);

  const hungerMeta = document.createElement('div');
  hungerMeta.style.cssText = 'font-size:10px;color:#999;margin-top:2px;display:none;';
  hungerCard.appendChild(hungerMeta);

  const feedRateEl = document.createElement('div');
  feedRateEl.style.cssText = 'font-size:10px;color:#81c784;margin-top:4px;font-family:monospace;';
  hungerCard.appendChild(feedRateEl);
  ...
  indicatorsGrid.appendChild(hungerCard);

  // === Restock Indicator ===
  const restockCard = createMinimalCard('🛒', 'Restock', 'rgba(66,165,245,0.15)');
  const restockSummary = document.createElement('div');
  restockSummary.style.cssText = 'font-size:10px;color:#999;display:none;';
  restockCard.appendChild(restockSummary);

  const restockRows = document.createElement('div');
  restockRows.style.cssText = 'display:flex;flex-direction:column;gap:3px;font-size:11px;color:#e0e0e0;';
  restockCard.appendChild(restockRows);
  ...
  for (const cat of SHOP_CATEGORY_DEFINITIONS) {
    ...
    const value = document.createElement('span');
    value.textContent = '...';
    value.style.cssText = 'font-family:monospace;color:#999;font-size:10px;';
    row.append(label, value);
  ...
  indicatorsGrid.appendChild(restockCard);
...
function refreshHeaderStats(): void {
  ...
  if (uiState.headerMeta) {
    ...
    const harvestStatusText = (harvestHeaderStatusText || uiState.harvestStatus?.textContent || '').trim();
    if (harvestStatusText) {
      metaParts.push(`🌾 ${harvestStatusText}`);
    } else {
      try {
        const harvest = getHarvestSummary();
        const readyCount = harvest?.readyCount ?? 0;
        metaParts.push(`🌾 Matches ready: ${readyCount}`);
      } catch {
        metaParts.push('🌾 Waiting for sunny weather...');
      }
    }
...
function createHowToUseSection(): HTMLElement {
  ...
  const features = [
    ...
    },
    {
      icon: '⭐',
      name: 'Auto-Favorite',
      desc: 'Automatically favorites gold and rainbow crops and pets when they appear in your inventory. (See Inventory Settings)'
    },
  ];
...
function createFeedSection(): HTMLElement {
  ...
  const { root, body } = createCard('🍖 Feed Tracking', {
    collapsible: true,
    startCollapsed: true,
  });
  ...
  const infoText = document.createElement('div');
  infoText.className = 'qpm-section-muted';
  infoText.textContent = 'Feed event tracking is available in the Statistics Dashboard.';
  infoText.style.padding = '8px 0';
  body.appendChild(infoText);
...
function createComingSoonSection(): HTMLElement {
  ...
  const { root, body } = createCard('🚧 Roadmap & Preview', {
  ...
  const groups: RoadmapGroup[] = [
    {
      ...
      items: [
        { name: 'Mutation Strategy Planner', description: 'Inventory-aware mutation guidance for multi-harvest plants. Coming Soon!' },
        { name: 'Lunar Event Tracker', description: 'Amber/Dawnlit event timing + success history. Coming Soon!' },
        { name: 'Weather Precision Alerts', description: 'Correct rain, snow, amber, and dawn detection for reminders. Coming Soon!' },
        { name: 'Garden Value + Decor', description: 'Full garden appraisal including decor bonuses. Coming Soon!' },
        { name: 'Mutation Forecast Fixes', description: 'No more false moon-event alerts after weather clears. Coming Soon!' },
      ],
    },
    {
      ...
      items: [
        ...
        { name: 'Quick Hutch Transfer', description: 'Pick-up/store-all controls with smart filters. Coming Soon!' },
        { name: 'Pet Earnings Monitor', description: 'Pet preset profit per hour simulations. Coming Soon!' },
    ...
    },
    {
      ...
      items: [
        { name: 'Respect Rules Reliability', description: 'Ensure feed-from-inventory selects the correct food. Coming Soon!' },
        { name: 'Weather Scan Improvements', description: 'Reliably detect weather variants for reminders. Coming Soon!' },
      ],
    },
  ];
  ...
  for (const group of groups) {
    ...
    groupTitle.style.color = 'var(--qpm-text)';
    body.appendChild(groupTitle);

    const grid = document.createElement('div');
    grid.className = 'qpm-coming-grid';

    for (const item of group.items) {
      const card = document.createElement('div');
      card.className = 'qpm-coming-card';
      ...
      if (item.description) {
        const desc = document.createElement('div');
        desc.className = 'qpm-section-muted';
...
function createHarvestSection(): HTMLElement {
  cfg.harvestReminder = {
    enabled: cfg.harvestReminder?.enabled ?? false,
    highlightEnabled: cfg.harvestReminder?.highlightEnabled ?? true,
    toastEnabled: cfg.harvestReminder?.toastEnabled ?? true,
    minSize: typeof cfg.harvestReminder?.minSize === 'number' ? cfg.harvestReminder.minSize : 80,
    selectedMutations: {
      ...HARVEST_MUTATION_DEFAULTS,
      ...(cfg.harvestReminder?.selectedMutations || {}),
    },
  };
  ...
  statusChip.textContent = cfg.harvestReminder.enabled ? 'Enabled' : 'Disabled';

  const { root, body } = createCard('🌾 Harvest Reminder', {
    collapsible: true,
    startCollapsed: true,
    subtitleElement: statusChip,
  });
  root.dataset.qpmSection = 'harvest-reminder';
  ...
  summary.style.cssText = 'color:#bbb;font-size:11px;line-height:1.4';
  summary.innerHTML = 'Matches: 0 • Value: 0<br>Waiting for sunny weather...';
  body.appendChild(summary);

  uiState.harvestStatus = status;
  uiState.harvestSummary = summary;

  harvestSummaryUnsubscribe = onHarvestSummary((data: HarvestSummary) => {
    updateHarvestSummaryUI(data);
  });
  harvestToastUnsubscribe = onHarvestToast((message: string) => {
    handleHarvestToast(message);
  });

  updateHarvestSummaryUI(getHarvestSummary());

  if (cfg.harvestReminder.enabled) {
    updateHarvestStatus(uiState.harvestStatus?.textContent ?? 'Waiting for sunny weather...');
  }
...
function createShopSection(): HTMLElement {
  ...
  const { root, body } = createCard('🛒 Auto Shop', {
    collapsible: true,
    startCollapsed: true,
    subtitleElement: statusChip,
  ...
  sStatus.className = 'qpm-section-muted';
  body.appendChild(sStatus);

  const countdownBox = document.createElement('div');
  countdownBox.style.cssText = 'margin-bottom:8px;padding:6px;background:#2b1f12;border-radius:4px;border:1px solid rgba(255,152,0,0.25);display:flex;flex-direction:column;gap:4px';

  const countdownValues = {} as Record<ShopCategory, HTMLSpanElement>;
  for (const cat of SHOP_CATEGORY_DEFINITIONS) {
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;justify-content:space-between;align-items:center;font-size:11px;color:#ddd';

    const labelSpan = document.createElement('span');
    labelSpan.textContent = `${cat.icon} ${cat.label}`;

    const valueSpan = document.createElement('span');
    valueSpan.textContent = 'off';
    valueSpan.style.fontFamily = 'monospace';
    valueSpan.style.color = '#777';

    row.append(labelSpan, valueSpan);
    countdownBox.appendChild(row);
    countdownValues[cat.key] = valueSpan;
  }
  ...
  import('../features/shopTracking')
    .then((module) => {
      ...
      module.setRestockInfoCallback((info: RestockInfo) => {
        latestRestockInfo = info;
        updateShopCountdownViews();
      });
  ...
  Promise.all([
    ...
    .then(([shopTrackingModule, shopStockModule]) => {
      ...
      for (const category of SHOP_CATEGORY_DEFINITIONS) {
        ...
        const categoryState = state.categories?.[category.key];
        ...
        const items = (Object.entries(categoryConfig) as Array<[string, AutoShopItemConfig]>).sort((a, b) => {
          const [idA, entryA] = a;
          const [idB, entryB] = b;
          const orderA = orderIndex.has(idA) ? orderIndex.get(idA)! : Number.MAX_SAFE_INTEGER;
          const orderB = orderIndex.has(idB) ? orderIndex.get(idB)! : Number.MAX_SAFE_INTEGER;
          if (orderA !== orderB) {
            return orderA - orderB;
          }
          const rankA = computeShopItemRank(entryA);
          const rankB = computeShopItemRank(entryB);
          if (rankA !== rankB) {
            return rankA - rankB;
          }
          const aLabel = entryA.displayLabel ?? entryA.label ?? idA;
          const bLabel = entryB.displayLabel ?? entryB.label ?? idB;
        ...
        catSection.style.cssText = 'margin-top:8px;padding:6px;background:#2a2a2a;border-radius:4px';

        const header = document.createElement('div');
        header.textContent = `${category.icon} ${category.label}`;
        header.style.cssText = 'font-weight:bold;margin-bottom:4px;font-size:11px;color:#eee';
        catSection.appendChild(header);

        for (const [itemId, entry] of items) {
          const enabled = Boolean(entry.enabled);
          const displayName = entry.displayLabel ?? entry.label ?? itemId;
          const itemBtn = btn(`${enabled ? '✓' : '✗'} ${displayName}`, () => {
            itemBtn.disabled = true;
            setItemEnabled(category.key, itemId, !enabled);
          });
          itemBtn.style.cssText = `width:100%;margin-bottom:2px;font-size:10px;background:${enabled ? '#4CAF50' : '#555'}`;
          itemBtn.dataset.itemId = itemId;
          itemBtn.dataset.shopCategory = category.key;
          catSection.appendChild(itemBtn);
        }
      ...
      if (container.children.length === 0) {
        const hint = document.createElement('div');
        hint.textContent = '⚠️ No items scanned yet. Click "Scan All Shops" above.';
        hint.style.cssText = 'color:#999;font-size:10px;margin-top:8px;text-align:center';
        container.appendChild(hint);
      }
    })
    .catch((error) => log('⚠️ Failed to refresh shop items', error));
}
...
function createWeatherTestSection(): HTMLElement {
  const testSection = document.createElement('div');
  testSection.style.cssText = 'margin-bottom:12px';

  const tTitle = document.createElement('div');
  tTitle.textContent = '🧪 Weather Test';
  tTitle.style.cssText = 'font-weight:bold;margin-bottom:6px;font-size:13px;color:#17a2b8;cursor:pointer;user-select:none';

  const tCollapse = document.createElement('div');
  tCollapse.style.cssText = 'display:none';
  tTitle.onclick = () => tCollapse.style.display = tCollapse.style.display === 'none' ? 'block' : 'none';

  const buttonRow = document.createElement('div');
  buttonRow.style.cssText = 'display:flex;gap:6px;margin-bottom:8px';

  const detectBtn = btn('🔍 Detect Weather', async () => {
    resultsArea.textContent = 'Detecting weather...\n';
    try {
      const result = await testWeatherDetection();
      resultsArea.textContent += `Weather detected: ${result}\n`;
    } catch (error) {
      resultsArea.textContent += `Error: ${error}\n`;
    }
  });
  detectBtn.style.cssText = 'flex:1;font-size:10px;padding:4px 6px';

  const sunnyBtn = btn('☀️ Sunny Key', () => {
    resultsArea.textContent += 'Triggering sunny keybind...\n';
    try {
      triggerSunnyKey();
      resultsArea.textContent += 'Sunny key triggered ✓\n';
    } catch (error) {
      resultsArea.textContent += `Error: ${error}\n`;
    }
  });
  sunnyBtn.style.cssText = 'flex:1;font-size:10px;padding:4px 6px';

  const weatherBtn = btn('🌧️ Weather Key', () => {
    resultsArea.textContent += 'Triggering weather keybind...\n';
    try {
      triggerWeatherKey();
      resultsArea.textContent += 'Weather key triggered ✓\n';
    } catch (error) {
      resultsArea.textContent += `Error: ${error}\n`;
    }
  });
  weatherBtn.style.cssText = 'flex:1;font-size:10px;padding:4px 6px';

  const resultsArea = document.createElement('div');
  resultsArea.style.cssText = 'background:#222;border:1px solid #444;padding:6px;border-radius:4px;font-family:monospace;font-size:10px;max-height:120px;overflow-y:auto;white-space:pre-wrap;color:#ccc';
  resultsArea.textContent = 'Click buttons to test weather detection and keybind triggers...\n';

  buttonRow.append(detectBtn, sunnyBtn, weatherBtn);
  tCollapse.append(buttonRow, resultsArea);
  testSection.append(tTitle, tCollapse);

  return testSection;
}

function createKeybindRow(label: string, value: string, onSet: (keyData: any) => void, onClear: () => void): HTMLElement {
  const row = document.createElement('div');
  row.style.cssText = 'display:flex;align-items:center;margin-bottom:6px;gap:8px';

  const labelEl = document.createElement('span');
  labelEl.textContent = label;
  labelEl.style.cssText = 'min-width:80px;color:#ddd;font-size:11px';
  ...
  if (value) {
    try {
      ...
      input.value = formatKeybind(data);
    } catch {
      input.value = '';
    }
  }

  input.style.cssText = 'flex:1;padding:3px 6px;border:1px solid #555;background:#333;color:#fff;border-radius:4px;font-size:11px;cursor:pointer';
  ...
  input.addEventListener('click', (e) => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      onSet(keyData);
      input.value = formatKeybind(keyData);
  ...
  clearBtn.style.cssText = 'padding:2px 6px;font-size:11px;background:#d32f2f;min-width:24px';

  row.append(labelEl, input, clearBtn);
  return row;
}
...
function updateHarvestStatus(text: string): void {
  harvestHeaderStatusText = text;
  if (uiState.harvestStatus) {
    uiState.harvestStatus.textContent = text;
  }
  refreshHeaderStats();
}

function updateHarvestSummaryUI(summary: HarvestSummary): void {
  if (!uiState.harvestSummary) return;

  const valueText = summary.totalValue > 0 ? `~${formatCoins(summary.totalValue)}` : '0';
  let html = `Matches: ${summary.readyCount} • Value: ${valueText}`;

  if (summary.highestValue) {
    const { species, value, size, breakdownText, totalMultiplierText } = summary.highestValue;
    const detail = breakdownText ? `${breakdownText} → total x${totalMultiplierText}` : `Total x${totalMultiplierText}`;
    html += `<br>⭐ ${species} — ${detail} • size ${size} (~${formatCoins(value)})`;
  } else {
    html += '<br>Waiting for sunny weather...';
  }

  uiState.harvestSummary.innerHTML = html;
  ...
  const statusText = summary.readyCount > 0 ? `Matches ready: ${summary.readyCount}` : 'Waiting for sunny weather...';
  updateHarvestStatus(statusText);

  refreshHeaderStats();
}

function handleHarvestToast(message: string): void {
  updateHarvestStatus(message);
  if (harvestStatusTimer) {
    window.clearTimeout(harvestStatusTimer);
  }
  harvestStatusTimer = window.setTimeout(() => {
    harvestStatusTimer = null;
    if (!cfg.harvestReminder?.enabled) {
      updateHarvestStatus('Disabled');
      return;
    }
...
async function testWeatherDetection(): Promise<string> {
  ...
  if (!canvas) {
    ...
    const root = hud ?? document.getElementById('App') ?? document;
    const canvases = Array.from(root.querySelectorAll('canvas'));
    throw new Error(`Weather canvas not found. Found ${canvases.length} canvas element(s) in HUD search roots.`);
  }

  let matchedSelector = 'automatic heuristics';
  for (const selector of WEATHER_CANVAS_SELECTORS) {
    const element = document.querySelector(selector);
    if (!element) {
      continue;
    }
    if (element === canvas || (element instanceof HTMLElement && element.contains(canvas))) {
