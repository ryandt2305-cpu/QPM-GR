...
import { getPetSpriteDataUrl, spriteExtractor, serviceReady } from '../sprite-v2/compat';
...
const spritesReadyPromise = serviceReady.then((svc) => {
  spritesReady = !!svc;
  if (spritesReady) {
    scheduleAchievementSpriteHydration();
    scheduleAchievementSpriteIdleWarmup();
...
let spriteSvc: SpriteService | null = null;
const spriteLookup = new Map<string, { cat: SpriteCategory; id: string }[]>();

function normalizeKey(name: string): string {
  return String(name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
}

void spritesReadyPromise.then((svc) => {
  spriteSvc = svc;
  if (!svc) return;
  const cats: SpriteCategory[] = ['item', 'decor', 'pet', 'seed', 'plant', 'tallplant', 'crop'];
  cats.forEach((cat) => {
    try {
      svc.list(cat).forEach((it) => {
        ...
        const arr = spriteLookup.get(key) ?? [];
        arr.push({ cat, id });
        spriteLookup.set(key, arr);
      });
    } catch {
      // ignore
    }
...
function resolveSprite(def: AchievementDefinition): ResolvedSprite {
  ...
  const fromService = (...names: string[]) => {
    if (!spriteSvc) return null;
    for (const name of names) {
      if (!name) continue;
      const key = normalizeKey(name);
      const matches = spriteLookup.get(key);
      const pick = matches && matches.length ? matches[0] : null;
      if (!pick) continue;
      const canvas = spriteSvc.renderToCanvas({ category: pick.cat as any, id: pick.id, mutations: [] });
      if (canvas) {
        ...
        el.style.cssText = `
          ...
          flex-shrink: 0;
        `;
        registerAchievementSprite(el, () => url);
        return { element: el, url } satisfies ResolvedSprite;
      }
    }
    return null;
  };
  ...
  const svcSprite = fromService(def.title, def.id, base, def.description);
  if (svcSprite) return svcSprite;

  return { element: null, url: null };
}
...
function pickSprite(def: AchievementDefinition): HTMLElement | null {
  ...
  placeholder.textContent = def.icon || '☆';
...
function buildBadgeSprite(def: AchievementDefinition, rarity: AchievementRarity, size: number = 52): HTMLElement {
  ...
  if (element) {
    ...
    if (fallback) {
      ...
      fill.style.cssText = `position:absolute; inset:0; border-radius: 10px; background:${gradient}; display:flex; align-items:center; justify-content:center; color:#f5f5f5; font-weight:800;`;
      fill.textContent = def.icon || '☆';
...
export function createAchievementsWindow(): AchievementsWindowState {
  ...
  try { triggerAchievementRecompute(); } catch (error) { log('Achievements recompute failed', error); }

  void spritesReadyPromise.then(() => {
    render();
    renderOneTime(state, oneTimeDefs);
    renderBadges(state, defsCache);
    hydrateAchievementSpritesWithin(state.root);
