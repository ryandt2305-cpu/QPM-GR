...
type MutationSpriteType = 'rainbow' | 'gold';
...
const SPRITE_HYDRATION_FRAME_BUDGET_MS = 12;  // Restored to 12ms - warmup system reduces on-demand rendering
const SPRITE_HYDRATION_MAX_PER_FRAME_DEFAULT = 4;  // Default batch size
const SPRITE_HYDRATION_MAX_PER_FRAME_LARGE_QUEUE = 8;  // Larger batch when queue is overwhelmed
const SPRITE_HYDRATION_INITIAL_SYNC_COUNT = 20;  // Pre-render first 20 sprites synchronously
...
function hydrateSpritesWithin(root: ParentNode | null): void {
  ...
  // **OPTIMIZATION**: Async micro-batched rendering for non-blocking display
  // This eliminates freezing when opening Pet Hub while sprites are still being cached
  const nodesToHydrate: HTMLElement[] = [];
  ...
  // Render sprites asynchronously in micro-batches to avoid blocking
  hydrateSpritesAsync(nodesToHydrate);
}
...
function processSpriteHydrationQueue(): void {
  ...
  // **OPTIMIZATION**: Increase batch size when queue is overwhelmed
  // If queue has 50+ items, process more per frame to catch up faster
  const maxPerFrame = spriteHydrationQueue.size >= 50
    ? SPRITE_HYDRATION_MAX_PER_FRAME_LARGE_QUEUE
    : SPRITE_HYDRATION_MAX_PER_FRAME_DEFAULT;
  ...
  for (const node of Array.from(spriteHydrationQueue)) {
    ...
    hydratedThisFrame += 1;
    if (hydratedThisFrame >= maxPerFrame || performance.now() - start >= SPRITE_HYDRATION_FRAME_BUDGET_MS) {
      break;
    }
  }
  if (spriteHydrationQueue.size) {
...
/**
 * Asynchronously hydrate sprites in micro-batches to avoid blocking the main thread
 * This eliminates the Pet Hub freeze caused by synchronous sprite rendering
 */
async function hydrateSpritesAsync(nodes: HTMLElement[]): Promise<void> {
  const BATCH_SIZE = 5; // Process 5 sprites per batch
  const YIELD_INTERVAL = 16; // Yield every 16ms (approximately 60fps)
  ...
  for (let i = 0; i < nodes.length; i += BATCH_SIZE) {
    ...
    // Yield to browser if we've processed a batch
    if (i + BATCH_SIZE < nodes.length) {
      const elapsed = performance.now() - batchStart;
      if (elapsed >= YIELD_INTERVAL) {
        // Use requestIdleCallback for truly non-blocking rendering
...
function getDisplaySprite(stats: SpriteStatsInput | null | undefined): string | null {
  ...
  for (const mutation of MUTATION_DISPLAY_PRIORITY) {
    if (mutations.has(mutation)) {
      const mutatedSprite = canvasToDataUrl(
        getPetSpriteWithMutations(speciesKey, [mutation === 'rainbow' ? 'Rainbow' : 'Gold'])
      );
      if (mutatedSprite) {
  ...
  return canvasToDataUrl(getPetSpriteCanvas(speciesKey));
}
...
export function renderPetHubWindow(root: HTMLElement): void {
  ...
  // Sprite warmup is now handled globally by the sprite-v2 system
  // No need for per-feature warmup calls
