...
// Ensure sprite sheets begin loading for inspector rendering
initSprites();

// Track sprite loading state
let spriteSheetsReady = false;
const spriteLoadPromise = (async () => {
  try {
    // Load critical sheets for inspector
    await spriteExtractor.loadSheetFromUrl('https://magicgarden.gg/version/436ff68/assets/tiles/pets.png', 'pets');
    await spriteExtractor.loadSheetFromUrl('https://magicgarden.gg/version/436ff68/assets/tiles/plants.png', 'plants');
    await spriteExtractor.loadSheetFromUrl('https://magicgarden.gg/version/436ff68/assets/tiles/seeds.png', 'seeds');
    spriteSheetsReady = true;
  } catch (error) {
    console.warn('[QPM] Failed to preload sprite sheets:', error);
  }
})();
...
function getItemSpriteUrl(name: string): string | null {
  ...
  if (typeof tileIndex !== 'number') return null;
  const tile = spriteExtractor.getTile(ITEM_SHEET, tileIndex);
  if (!tile) return null;
  return canvasToDataUrlSafe(tile);
}
...
function getEggSpriteUrl(eggId: string): string | null {
  ...
  for (const variant of variants) {
    const canvas = spriteExtractor.getPetSprite(variant);
...
function getMutatedCropSpriteUrl(species: string | null | undefined, mutations: any[], tileId?: string | number | null): string | null {
  ...
  const baseCanvas = spriteExtractor.getCropSprite(normalized);
  if (!baseCanvas) return getCropSpriteDataUrl(normalized);
  const ordered = mutations.map((m) => String(m || '').toLowerCase()).filter(Boolean).slice(0, 4);
  const rendered = ordered.length ? spriteExtractor.renderPlantWithMutations(baseCanvas, ordered) : baseCanvas;
  return canvasToDataUrlSafe(rendered);
}
...
function getSeedSpriteUrl(seedName: string): string | null {
  ...
  const canvas = spriteExtractor.getSeedSprite(normalized);
  return canvasToDataUrlSafe(canvas);
}
...
async function renderInventoryPane(view: PlayerView, isFriend: boolean, privacy: PlayerView['privacy']): Promise<void> {
  // Wait for sprite sheets to be ready
  await spriteLoadPromise;
  
  const allowInventory = true;
  ...
  const pushItem = (item: any, fallbackKey?: string): void => {
    ...
    // Filter out non-item entries like FavoritedItemIDs
    ...
    // Extract egg rarity from eggId (e.g., "LegendaryEgg" -> "Legendary")
    let eggRarity = null;
    if (isEgg && item.eggId) {
    ...
    // Normalize tulip variants (OrangeTulip, WhiteTulip, etc.) to just "Tulip"
    ...
    // Calculate pet level/strength using direct XP calculation like Pet Hub
    ...
    // rainbow/gold pets need mutation filter applied
    ...
    if (isSeed && species) {
      ...
      // Eggs are in pets.png sheet (same location as pets)
      sprite = item.eggId ? getEggSpriteUrl(String(item.eggId)) : null;
    } else if (isPet && species) {
      // Get pet sprite - normalize species name for lookup
      const normalized = normalizePetSpecies(String(species));
      
      // Use getPetSpriteDataUrl directly (has caching, works like Pet Hub)
      sprite = getPetSpriteDataUrl(normalized);
      
      // Apply mutation filter if needed
      if (sprite && petMut) {
        const petCanvas = spriteExtractor.getPetSprite(normalized);
        if (petCanvas) {
          const withMut = applyCanvasFilter(petCanvas, petMut);
          sprite = canvasToDataUrlSafe(withMut) || sprite; // Keep original if filter fails
        }
      }
    } else if (species) {
      sprite = getMutatedCropSpriteUrl(String(species).toLowerCase(), item.mutations || []);
    } else {
      sprite = getItemSpriteUrl(lowerLabel) || getItemSpriteUrl(baseLabel.toLowerCase());
    }
  ...
  const cards = collected.map(item => {
    if (item.isPet) {
      // Pet card - vertical format like game inventory
    ...
    // Non-pet item card - vertical format like game inventory
  ...
  // Render Pet Hutch from storages array
  ...
  if (hutch && hutch.items && hutch.items.length > 0) {
    const hutchPets = hutch.items.map((pet: any) => {
      const species = pet.petSpecies || pet.species;
      const normalized = normalizePetSpecies(String(species));
      const name = pet.name || friendlyName(species);
      
      // Get sprite with mutations applied
      let sprite = getPetSpriteDataUrl(normalized);
      ...
      // Calculate level using direct XP calculation like Pet Hub
      let strength = 50; // Base strength for 0 XP pets
  ...
  setPaneContent('pr-inventory-content', `
    <div class="pr-section">
      <div class="pr-section-head">🎒 Inventory (${collected.length} items)</div>
      <div class="pr-card-grid">${cards || '<div class="pr-pane-placeholder">Inventory payload empty or private.</div>'}</div>
    </div>
    ${hutchHtml}
  `);
}
